...
def create_app(test_config=None):
    ...
    def get_paginated_questions():
        try:
            ...
            questions_paginated = paginate_questions(questions_rows,
                                                     page_index)
        except Exception:
            # exception details
            print('\n * Error:\n', sys.exc_info(), '\n')
    ...
    def post_search_or_post_new_question():
        ...
        if search_term is not None:
            ...
            try:
                ...
                if (new_question_question is not None and
                        new_question_answer is not None and
                        new_question_difficulty is not None and
                        new_question_category is not None):
                    ...
                    questions_rows = Question.query.order_by(Question.id).all()
                    total_questions = len(questions_rows)
                    last_page_index = (total_questions
                                       // QUESTIONS_PER_PAGE) + 1
                    questions_paginated = paginate_questions(
                                            questions_rows, last_page_index)
            ...
            if (new_question_question is None or
                    new_question_answer is None or
                    new_question_difficulty is None or
                    new_question_category is None):
                # Unprocessable error
                abort(422)
